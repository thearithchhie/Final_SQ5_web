...
function getStatusColumn($column, $attributeName) {
    if (isset($column)) {
        if ($column[$attributeName]) {
            return "<input  class='change_status'  type='checkbox' data-id='".$column->id."' checked />";
        } else {
            return "<input   class='change_status' type='checkbox' data-id='".$column->id."' />";
        }
    }
}
...
function getLocalAddress($request){
    if ($request->has(['lng', 'lat'])){
...
function getCart($user_id){
    ...
    $param = new Request();
    $cartRepo = App::make('App\Repositories\CartRepository');
    $param['with'] = 'product;product.market;options';
    $param['search'] = 'user_id:'.$user_id ; 
    try{
        $cartRepo->pushCriteria(new RequestCriteria($param));
        $cartRepo->pushCriteria(new LimitOffsetCriteria($param));
