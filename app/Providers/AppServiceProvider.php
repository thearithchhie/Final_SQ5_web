...
use App\Models\{Field, Order, Category};
...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        view()->composer('*', function ($view) {
            if (Auth::check()) {
                $view->with('cart', getCart(Auth::user()->id));
            } else {
                $view->with('cart', null);
            }
        });
        View::share('categories',Category::categoryActive()->get());
        View::share('orders', Order::paginate(4));
        View::share('location_address', null);
        View::share('fields', Field::select(['id', 'name'])->get());
